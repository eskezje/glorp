    ...
    unsafe {
        ...
        loop {
            let name = String::from_utf16_lossy(&pe.szExeFile)
                .trim_matches('\0')
                .to_lowercase();
            if name.contains("msedgewebview2.exe")
                && pe.th32ParentProcessID == browser_pid
                && pe.th32ProcessID != browser_pid
            {
        ...
        let remote_mem =
            VirtualAllocEx(hproc, None, bytes, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        ...
        let loadlib =
            GetProcAddress(h_kernel32, s!("LoadLibraryW")).ok_or_else(|| Error::from(E_POINTER))?;
